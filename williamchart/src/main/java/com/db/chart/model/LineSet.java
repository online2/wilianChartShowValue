...
public class LineSet extends ChartSet {
    ...
    private static final float LINE_VALUE_SEIZ = 8;
    ...
    public LineSet(@NonNull String[] labels, @NonNull float[] values) {
        ...
        if (labels.length != values.length) throw new IllegalArgumentException("Arrays size doesn't match.");
        checkNotNull(labels);
        checkNotNull(values);
    ...
    private void init() {
        ...
        mValueTextSize = Tools.fromDpToPx(LINE_VALUE_SEIZ);
    ...
    /**
     * Retrieve line value color.
     *
     * @return Line value  color.
     */
    ...
    public LineSet setGradientFill(@NonNull @Size(min = 1) int colors[], float[] positions) {
        ...
        if (colors.length == 0) throw new IllegalArgumentException("Colors argument can't be null or empty.");
    ...
    public LineSet endAt(@IntRange(from = 0) int index) {
        ...
        if (index < mBegin) throw new IllegalArgumentException("Index cannot be lesser than the start entry " + "defined in beginAt(index).");
